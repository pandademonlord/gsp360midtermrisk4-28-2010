...
#include <math.h>	// sin(), cos() TODO remove dependency...
#include "glut.h"	// TODO remove OpenGL dependency

/** the standard data type used in scalar calculations */
#define V2D_SIN		sin
#define V2D_COS		cos
...
public:
	...
	/** to make one dimensional values for this class's type */
	typedef VTYPE type;
	...
	inline VTYPE getField(const int & a_dimensionField)const{getDimensions()[a_dimensionField];}
	 ...
	 */
	static const V2D & ZERO()			{static V2D<VTYPE> ZERO(0,0);			return ZERO;}
	static const V2D & ZERO_DEGREES()	{static V2D<VTYPE> ZERODEGREES(1,0);	return ZERODEGREES;}
	 ...
	 */
	inline VTYPE sign(const V2D<VTYPE> & v2) const{return (x*v2.y)-(y*v2.x);}// > (y*v2.x > x*v2.y)?-1:1;}
	 ...
	 */
	inline void truncate(VTYPE a_maxLength)
	{
	...
	inline bool equals(const V2D<VTYPE> & v){return x == v.x && y == v.y;}
	...
	{
		V2D_GLVERTEX_METHOD((VTYPE*)this);
	}
	...
	/**
	 * draw an OpenGL line from (0,0) to (V2D.x, V2D.y) 
	...
	inline bool glDrawTo(const V2D<VTYPE> & a_next) const
	{
		glBegin(GL_LINES);
		glVertex();
		a_next.glVertex();
	 ...
	 */
	static inline void glVertexList(V2D<VTYPE> * a_list, const int & a_count)
	{
	...
	void glDrawString(char * a_string)
	{
		glRasterPos2f (x, y);
		unsigned int i;
		for (i = 0; a_string[i]; i++)
			glutBitmapCharacter (GLUT_BITMAP_HELVETICA_10, a_string[i]);
	}
